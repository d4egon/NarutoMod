...
{
	public class AmaterasuFire : AttachableThing, ISizeReporter
	{
		...
		public const float MaxFireSize = 4.75f;

		private const int TicksToBurnFloor = 7500;

		private const int ComplexCalcsInterval = 150;

		private const float CellIgniteChancePerTickPerSize = 0.005f;

		private const float MinSizeForIgniteMovables = 0.1f;

		private const float FireBaseGrowthPerTick = 0.00015f;

		private static readonly IntRange SmokeIntervalRange = new IntRange(130, 200);

		private const int SmokeIntervalRandomAddon = 10;

		private const float BaseSkyExtinguishChance = 0.0f;

		private const int BaseSkyExtinguishDamage = 0;

		private const float HeatPerFireSizePerInterval = 160f;

		private const float HeatFactorWhenDoorPresent = 0.45f;
		...
		public override void Tick()
		{
			...
			if (AmaterasuFire.lastFireCountUpdateTick != Find.TickManager.TicksGame)
			{
				AmaterasuFire.fireCount = base.Map.listerThings.ThingsOfDef(this.def).Count;
				AmaterasuFire.lastFireCountUpdateTick = Find.TickManager.TicksGame;
			}
			if (this.sustainer != null)
			{
			...
			if (AmaterasuFire.fireCount < 15 && this.fireSize > 0.7f && Rand.Value < this.fireSize * 0.01f)
			{
				SovereignNarutoMod.SNM_FleckMaker.ThrowAmaterasuMicroSparks(this.DrawPos, base.Map);
			}
			if (this.fireSize > 1f)
			{
		...
		{
			if (AmaterasuFire.fireCount < 15)
			{
                RimWorld.FleckMaker.ThrowSmoke(this.DrawPos, base.Map, this.fireSize);
			}
			if (this.fireSize > 0.5f && this.parent == null)
			{
                RimWorld.FleckMaker.ThrowFireGlow(base.Position.ToVector3Shifted(), base.Map, this.fireSize);
			}
			...
			num = 1f - num;
			this.ticksUntilSmoke = AmaterasuFire.SmokeIntervalRange.Lerped(num) + (int)(10f * Rand.Value);
		}

		private void DoComplexCalcs()
		{
			bool flag = false;
			AmaterasuFire.flammableList.Clear();
			...
			if (!base.Position.GetTerrain(base.Map).extinguishesFire)
			{
				if (this.parent == null)
				{
					...
					for (int i = 0; i < list.Count; i++)
					{
						...
						if (statValue >= 0.01f)
						{
							AmaterasuFire.flammableList.Add(list[i]);
							if (statValue > this.flammabilityMax)
							{
								this.flammabilityMax = statValue;
							}
							if (this.parent == null && this.fireSize > 0.4f && list[i].def.category == ThingCategory.Pawn && Rand.Chance(AmaterasuFireUtility.ChanceToAttachFireCumulative(list[i], 150f)))
							{
				...
				{
					AmaterasuFire.flammableList.Add(this.parent);
			...
			else if (AmaterasuFire.flammableList.Count > 0)
			{
				thing = AmaterasuFire.flammableList.RandomElement<Thing>();
			}
			else
			{
				thing = null;
		...
		protected void TrySpread()
		{
			...
			if (Rand.Chance(AmaterasuFireUtility.ChanceToStartFireIn(intVec, base.Map)))
			{
				if (!flag)
				{
				...
				{
					AmaterasuFireUtility.TryStartFireIn(intVec, base.Map, 0.1f);
				}
			}
		}
	}
